            ...
            <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
              ...
              <div className="p-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-2 text-[15px]">
                  <h3 className="text-xl font-semibold text-gray-900">{t('order_details.order_details', 'Order Details')}</h3>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
                  ...
                  <div className="lg:col-span-2 border border-gray-200 rounded-lg grid grid-cols-1 md:grid-cols-2">
                    ...
                    <div className="p-6 border-b md:border-b-0 md:border-r border-gray-200">
                      <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">{t('order_details.billing_address', 'Billing Address')}</h4>
                      ...
                      <div className="space-y-1">
                        <p className="text-xs text-gray-400 uppercase font-medium">{t('order_details.email', 'Email')}</p>
                        <p className="text-sm text-gray-800">{order.user?.email || "N/A"}</p>
                      </div>
                      <div className="space-y-1 mt-3">
                        <p className="text-xs text-gray-400 uppercase font-medium">{t('order_details.phone', 'Phone')}</p>
                    ...
                    {/* Shipping Address */}
                    <div className="p-6">
                      <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">{t('order_details.shipping_address', 'Shipping Address')}</h4>
                      <h3 className="text-base font-semibold text-gray-900 mb-2">
                  ...
                  <div className="border border-gray-200 rounded-lg p-6">
                    <div className="flex justify-between items-center mb-6">
                      <div>
                        <p className="text-xs text-gray-400 uppercase font-medium mb-1">{t('order_details.order_id', 'Order ID:')}</p>
                        <p className="text-base font-semibold text-gray-900">#{order._id.substring(0, 4)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-400 uppercase font-medium mb-1">{t('order_details.payment_method', 'Payment Method:')}</p>
                    ...
                    <div className="border-t border-gray-100 pt-5 space-y-4">
                      ...
                      <div className="flex justify-between items-center pt-2">
                        <span className="text-lg text-gray-900">{t('order_details.total', 'Total')}</span>
                        ...
                        <div key={index} className="relative z-10 text-center">
                          ...
                          {index !== trackingSteps.length - 1 && (
                            <div className="hidden md:block absolute top-7 left-1/2 w-full h-[4px] bg-gray-100 rounded-full z-0">
                              <div
                                className={`h-full rounded-full transition-all duration-700 ease-in-out ${isCompleted ? 'bg-[#00B207] w-full' : 'w-0'
                                  }`}
                              />
                            </div>
                          )}
                          ...
                          {index !== trackingSteps.length - 1 && (
                            <div className="md:hidden absolute top-14 left-[26px] bottom-[-40px] w-[4px] bg-gray-100 rounded-full z-0">
                              <div
                                className={`w-full rounded-full transition-all duration-700 ease-in-out ${isCompleted ? 'bg-[#00B207] h-full' : 'h-0'
                                  }`}
                              />
                            </div>
                          )}
                          ...
                          <div className="relative z-10 flex flex-row md:flex-col items-center md:justify-center gap-5 md:gap-3">
                            ...
                            <div className="text-left md:text-center mt-1">
                              <h4 className={`text-[15px] font-semibold transition-colors duration-300 ${isActive || isCompleted ? 'text-gray-900' : 'text-gray-400'
                                }`}>
                                {step.title}
                              </h4>
